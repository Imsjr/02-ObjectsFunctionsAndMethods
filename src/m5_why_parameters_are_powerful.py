...
def main():
    ...
    even_better_draw_circles(rg.Point(100, 50), 10, 2,'red',10)
    even_better_draw_circles(rg.Point(-200, 0), 20, 15, 'blue', 5)
...
def better_draw_circles(point,length):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(length * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point,length,number_of_circles,pen_color,pen_thickness):
    """
        Starts out the same as the   draw_circles   function defined ABOVE.
        You Will make it an IMPROVED, MORE POWERFUL function per the above _TODO_.
        """
    turtle = rg.SimpleTurtle()
    turtle.pen = rg.Pen(pen_color,pen_thickness)
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)
